...
########################################
# SNP keep-list ID-style normalization
########################################

ibis_normalize_keep_list_to_bim_ids() {
    local keep_file="$1"
    local bim_file="$2"
    local out_file="$3"
    local label="${4:-SNP keep list}"

    [[ -s "$keep_file" ]] || die "$label missing or empty: $keep_file"
    [[ -s "$bim_file" ]] || die "target BIM missing or empty for $label normalization: $bim_file"
    mkdir -p "$(dirname "$out_file")"

    awk -v label="$label" '
        BEGIN {FS="[ \t]+"; OFS="\t"}
        function stripchr(x) { sub(/^chr/, "", x); sub(/^CHR/, "", x); return x }
        function key_from_id(id,    a,c,p) {
            if (id !~ /:/) return ""
            split(id, a, ":")
            c=stripchr(a[1]); p=a[2]
            if (c == "" || p == "") return ""
            return c ":" p
        }
        FNR == NR {
        ...
        $1 ~ /^#/ {next}
        {
            ...
            if (k != "" && (k in target)) {
                mapped=target[k]
                if (mapped != raw) converted++
            } else if (!(raw in target_id)) {
                unresolved++
            }
            if (!seen[mapped]++) print mapped
            total++
        }
        END {
            print "[WARN] " label " IDs normalized against target BIM before --extract" > "/dev/stderr"
            print "[WARN] " label " normalization summary: total=" total ", converted=" converted+0 ", unresolved_or_nonmatching=" unresolved+0 > "/dev/stderr"
        }
    ' "$bim_file" "$keep_file" > "$out_file"

    [[ -s "$out_file" ]] || die "normalized $label is empty: $out_file"
}

ibis_normalize_keep_list_to_vcf_chrpos() {
    local keep_file="$1"
    local vcf_file="$2"
    local out_file="$3"
    local label="${4:-SNP keep list}"
    ...
    [[ -s "$keep_file" ]] || die "$label missing or empty: $keep_file"
    [[ -s "$vcf_file" ]] || die "target VCF missing or empty for $label normalization: $vcf_file"
    mkdir -p "$(dirname "$out_file")"

    local chr_order
    chr_order=$(mktemp)
    write_vcf_chrom_order_file "$vcf_file" "$chr_order"
    [[ -s "$chr_order" ]] || die "could not read chromosome IDs from VCF for $label normalization: $vcf_file"
    ...
    awk -v label="$label" '
        ...
        function map_chr(c,    s,cc) {
            ...
            s=stripchr(c)
            if (s in bybase) return bybase[s]
            cc="chr" s
            if (cc in exact) return cc
            cc="CHR" s
            if (cc in exact) return cc
            if (s == "M" && ("MT" in exact)) return "MT"
            if (s == "MT" && ("M" in exact)) return "M"
            if (s == "M" && ("chrM" in exact)) return "chrM"
            if (s == "MT" && ("chrMT" in exact)) return "chrMT"
            if (s == "M" && ("chrMT" in exact)) return "chrMT"
            if (s == "MT" && ("chrM" in exact)) return "chrM"
            return ""
        }
        FNR == NR {
            exact[$1]=1
            s=stripchr($1)
            if (!(s in bybase)) bybase[s]=$1
            next
        }
        NF == 0 {next}
        $1 ~ /^#/ {next}
        ...
        END {
            print "[WARN] " label " IDs normalized to target VCF chromosome style before --extract" > "/dev/stderr"
            print "[WARN] " label " normalization summary: total=" total ", converted=" converted+0 ", unresolved_or_non_chrpos=" unresolved+0 > "/dev/stderr"
        }
    ' "$chr_order" "$keep_file" > "$out_file"

    rm -f "$chr_order"
    [[ -s "$out_file" ]] || die "normalized $label is empty: $out_file"
}

ibis_prepare_extract_for_input() {
    local input="$1"
    local format="$2"
    local work_prefix="$3"
    local keep_file="${4:-}"

    [[ -n "$keep_file" ]] || return 0
    [[ -s "$keep_file" ]] || die "SNP keep/extract file missing or empty: $keep_file"

    local norm_keep="${work_prefix}.extract.normalized.txt"
    local src_prefix
    ...
    case "$format" in
        plink)
            ...
            ibis_normalize_keep_list_to_bim_ids "$keep_file" "${src_prefix}.bim" "$norm_keep" "IBIS/lowpass SNP keep list"
            echo "$norm_keep"
            ;;
        vcf|bcf)
            ibis_normalize_keep_list_to_vcf_chrpos "$keep_file" "$input" "$norm_keep" "IBIS/lowpass SNP keep list"
            echo "$norm_keep"
            ;;
        *)
            echo "$keep_file"
...
ibis_prepare_plink_from_input() {
    ...
    local src_prefix
    local tmp_prefix
    local extract_file="${IBIS_EXTRACT:-}"
    ...
    case "$format" in
        vcf|bcf)
            # Important:
            # Do NOT run --extract in the same PLINK2 command as --set-all-var-ids.
            # PLINK2 applies --extract against the incoming/original variant IDs,
            # so a chr:pos keep-list may match zero variants if the original VCF IDs
            # are rsIDs, ".", or another style.
            #
            # Correct order:
            #   1) Convert VCF/BCF -> temporary PLINK and standardize variant IDs.
            #   2) Normalize the keep-list against the temporary .bim.
            #   3) Run --extract on the temporary PLINK prefix.
            ibis_check_file "$input"

            tmp_prefix="${out_prefix}.allvars"

            local -a convert_cmd
            if [[ "$format" == "vcf" ]]; then
                convert_cmd=("$IBIS_PLINK2" --vcf "$input" --max-alleles 2)
            else
                convert_cmd=("$IBIS_PLINK2" --bcf "$input" --max-alleles 2)
            fi

            [[ -n "$IBIS_SET_ALL_VAR_IDS" ]] && convert_cmd+=(--set-all-var-ids "$IBIS_SET_ALL_VAR_IDS")
            convert_cmd+=(--make-bed --out "$tmp_prefix")

            echo "[IBIS] PLINK2 convert command: ${convert_cmd[*]}"
            "${convert_cmd[@]}"

            ibis_check_plink_prefix "$tmp_prefix"

            if [[ -n "$extract_file" ]]; then
                [[ -s "$extract_file" ]] || die "SNP keep/extract file missing or empty: $extract_file"

                local normalized_extract="${out_prefix}.extract.normalized.txt"
                ibis_normalize_keep_list_to_bim_ids \
                    "$extract_file" \
                    "${tmp_prefix}.bim" \
                    "$normalized_extract" \
                    "IBIS/lowpass SNP keep list"

                local -a extract_cmd
                extract_cmd=("$IBIS_PLINK2" --bfile "$tmp_prefix" --extract "$normalized_extract" --make-bed --out "$out_prefix")

                echo "[IBIS] PLINK2 extract command: ${extract_cmd[*]}"
                "${extract_cmd[@]}"
            else
                cp "${tmp_prefix}.bed" "${out_prefix}.bed"
                cp "${tmp_prefix}.bim" "${out_prefix}.bim"
                cp "${tmp_prefix}.fam" "${out_prefix}.fam"
                [[ -s "${tmp_prefix}.log" ]] && cp "${tmp_prefix}.log" "${out_prefix}.convert.log" || true
            fi
            ;;
...
lowpass_prepare_highpass_for_unit() {
    ...
    if [[ -n "$keep_file" ]]; then
        lowpass_check_file "$keep_file" "SNP keep file"
        local normalized_keep="${out_prefix}.extract.normalized.txt"
        ibis_normalize_keep_list_to_bim_ids "$keep_file" "${highpass_prefix}.bim" "$normalized_keep" "high-pass SNP keep list"
        cmd+=(--extract "$normalized_keep")
    fi
...
init_lowpass_collection() {
    ...
    # Resolve optional simulation-id templates in run/output paths.
    # Supported placeholders:
    #   {n}, {N}, %n
    # Example:
    #   RUN_ID=cousin_d3_sim_{n}
    #   lowpass-merge 1  ->  RUN_ID=cousin_d3_sim_1
    #
    # Important:
    # Do not nest parameter expansion such as:
    #   ${LOWPASS_COLLECTION_DIR:-${OUT_DIR}/lowpass_merge/${RUN_ID}}
    # because some shells/contexts can parse the closing brace ambiguously and
    # leave a trailing "}" in the generated path. Build defaults in separate
    # steps instead.
    if [[ -z "${RUN_ID:-}" ]]; then
        RUN_ID="sim_{n}"
    fi
    RUN_ID=$(lowpass_expand_template "$RUN_ID" "$n")

    if [[ -z "${LOWPASS_COLLECTION_DIR:-}" ]]; then
        LOWPASS_COLLECTION_DIR="${OUT_DIR}/lowpass_merge/${RUN_ID}"
    else
        LOWPASS_COLLECTION_DIR=$(lowpass_expand_template "$LOWPASS_COLLECTION_DIR" "$n")
    fi

    if [[ -z "${LOWPASS_WORK_DIR:-}" ]]; then
        LOWPASS_WORK_DIR="${TMP_DIR}/lowpass_merge/${RUN_ID}"
    else
        LOWPASS_WORK_DIR=$(lowpass_expand_template "$LOWPASS_WORK_DIR" "$n")
    fi

    if [[ -z "${LOWPASS_LOG_DIR:-}" ]]; then
        LOWPASS_LOG_DIR="${LOG_DIR}/lowpass_merge/${RUN_ID}"
    else
        LOWPASS_LOG_DIR=$(lowpass_expand_template "$LOWPASS_LOG_DIR" "$n")
    fi

    # These two may also be supplied in config, so expand templates here too.
    if [[ -z "${LOWPASS_UNIT_MANIFEST:-}" ]]; then
        LOWPASS_UNIT_MANIFEST="${LOWPASS_COLLECTION_DIR}/unit_manifest.tsv"
    else
        LOWPASS_UNIT_MANIFEST=$(lowpass_expand_template "$LOWPASS_UNIT_MANIFEST" "$n")
    fi

    if [[ -z "${LOWPASS_STATUS_DIR:-}" ]]; then
        LOWPASS_STATUS_DIR="${LOWPASS_LOG_DIR}/status"
    else
        LOWPASS_STATUS_DIR=$(lowpass_expand_template "$LOWPASS_STATUS_DIR" "$n")
    fi
...

    {
        ...
        echo "LOWPASS_LOG_DIR=$LOWPASS_LOG_DIR"
        echo "LOWPASS_UNIT_MANIFEST=$LOWPASS_UNIT_MANIFEST"
        echo "LOWPASS_STATUS_DIR=$LOWPASS_STATUS_DIR"
    ...
    lowpass_msg "RUN_ID=$RUN_ID"
    lowpass_msg "collection dir: $LOWPASS_COLLECTION_DIR"
    lowpass_msg "work dir: $LOWPASS_WORK_DIR"
    lowpass_msg "log dir: $LOWPASS_LOG_DIR"
    lowpass_msg "unit manifest: $LOWPASS_UNIT_MANIFEST"
    lowpass_msg "status dir: $LOWPASS_STATUS_DIR"
}
