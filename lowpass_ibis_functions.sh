...
init_lowpass_collection() {
    ...
    RUN_ID=$(lowpass_expand_template "${RUN_ID:-sim_{n}}" "$n")

    LOWPASS_COLLECTION_DIR=$(lowpass_expand_template "${LOWPASS_COLLECTION_DIR:-${OUT_DIR}/lowpass_merge/${RUN_ID}}" "$n")
    LOWPASS_WORK_DIR=$(lowpass_expand_template "${LOWPASS_WORK_DIR:-${TMP_DIR}/lowpass_merge/${RUN_ID}}" "$n")
    LOWPASS_LOG_DIR=$(lowpass_expand_template "${LOWPASS_LOG_DIR:-${LOG_DIR}/lowpass_merge/${RUN_ID}}" "$n")

    # These two may also be supplied in config, so expand templates here too.
    LOWPASS_UNIT_MANIFEST=$(lowpass_expand_template "${LOWPASS_UNIT_MANIFEST:-${LOWPASS_COLLECTION_DIR}/unit_manifest.tsv}" "$n")
    LOWPASS_STATUS_DIR=$(lowpass_expand_template "${LOWPASS_STATUS_DIR:-${LOWPASS_LOG_DIR}/status}" "$n")
